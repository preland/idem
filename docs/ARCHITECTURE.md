...
| **Text has one scale, and two consumers** | `engine/gfx/plane/text/` | `txt_ui()` is derived from the *window*, which is right for the editor's UI and probably wrong for a game's HUD — a game draws in stage units and its text should scale with the stage. Nothing is broken today because the editor is the only consumer; a game with a HUD will need `txt_draw_big` with its own factor, or the scale will need to become a parameter. |
